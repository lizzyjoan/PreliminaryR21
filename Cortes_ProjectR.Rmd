...
Step 9: Clustering method and visualization  
...
```{r Basic_Human_Patterns_Boxplot}
...
```{r eval=FALSE, include=FALSE}
ggsave("/Users/eramsey/Desktop/R21_210302/PreliminaryR21/210629_human_NMF_prepost_pattern_boxplot.pdf", human_NMF_boxplot)
...
Heatmap(mouse_featloadings_long)

#cogaps - heatmap of original data clustered by pattern markers statistic
AP_mouse_res <- results()
plotPatternMarkers(AP_mouse, as.matrix(conv_mouse), patternPalette = "PiYG", samplePalette = "BrBG")
...
human Gene/Feature NMF Matrix Heatmap in ggplot
```{r}
...
```
Clustering human data for heatmap
```{r}
#using wide-form of data to cluster
#top20_human_matrix <- as.numeric(top20_human_featloadings)
par(mar=c(7,4,4,2)+0.1) 
human_nmf_heatmap2 <-  heatmap.2(top20_human_featloadings_matrix,col=viridis, trace='none', margin=c(3,5),
distfun=function(c) as.dist(1-cor(t(c))) ,
cexCol=1,cexRow=.5,scale = "row",
hclustfun=function(x) hclust(x, method="average"), main = "Top 20 Human NMF Avg Gene Weights"
)
# Error in plot.new() : figure margins too large

#human_nmf_heatmap2 <-  
  
png(file = "/Users/eramsey/Desktop/R21_210302/PreliminaryR21/210628_Human_NMF_Variance_GeneWeights.png")  
heatmap.2(top20_var_human_featloadings_matrix,col=viridis, trace='none', margin=c(8,10),
distfun=function(c) as.dist(1-cor(t(c))) ,
cexCol=1.5,cexRow=1.4,scale = "row", xlab = "NMF Patterns",
hclustfun=function(x) hclust(x, method="average"), main = "Most Variable Genes Across NMF Patterns"
)
dev.off()

ggsave(file = "210628_Human_NMF_Top20Variance_GeneWeights.pdf", human_nmf_heatmap2)
```
...
```{r eval=FALSE, include=FALSE}
ggsave("210621_NMF_human_featloadings_heatmap.pdf", human_featloadings_heatmap)
```
...
Scatterplot Projected Pattern Comparison
```{r}
projection_colors <- scale_colour_manual(values = c("red","green","blue","black"),
guide = guide_legend(title="Lineage"))

projection_plot <- ggplot(projection_NMF, aes(x = Pattern_1, y = Pattern_2, colour = PrePost)) +
  geom_point(size=5) + 
  projection_colors + 
  theme(legend.position = c(0,0), legend.justification = c(0,0), panel.background = element_rect(fill = "white"), legend.direction = "horizontal")
projection_plot
...
Heatmap to Compare Pre/Post Projected Patterns
```{r}
NMF_projection_long$log.weight <- log(NMF_projection_long$Factors)

NMF_projection_heatmap <- ggplot(NMF_projection_long, mapping = aes(x = Exercise_Pattern, y = Factors, fill = log.weight)) + 
  geom_tile() +
  labs(title = "NMF Projection") +
  xlab(label = "Patterns") + 
  ylab(label = "Projected Human Samples") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5), axis.text.y = element_text(size = 5)) + 
  scale_fill_viridis(option = "plasma")
NMF_projection_heatmap
...
projected_PCA <- ggplot(PCA_projectr_df, aes(x = PC1, y = PC2, colour = PrePost)) + 
  ...
  ggtitle("Mouse Principal Components Projected onto Human Data")
projected_PCA
```

```{r eval=FALSE, include=FALSE}
ggsave("210629_PCA_Method_Mouse_Projected_onto_Human_new.pdf", projected_PCA)
```
...
<br>
Select each pattern and combine with its standard deviation ((note: might not have been necessary to get standard deviation...))  
```{r eval=FALSE, include=FALSE}
...
#delete afterwards, using as reference
```{r eval=FALSE, include=FALSE}
gost_res_mouse <- gost(query = pattern3_mouse_names, organism = "hsapiens", ordered_query = FALSE, significant = TRUE, exclude_iea = FALSE, measure_underrepresentation = FALSE, evcodes = FALSE, 
...
#results for each pattern was saved
#mouse_pattern5_rkc <- gost_res_mouse$result
mouse_pattern3_GO <- gost_res_mouse$result

gost_mouse_pattern3_top150gene_fea <- gost_res_mouse$result
#conversion to make this dataframe writable for csv
gost_mouse_pattern3_top150gene_fea <- apply(gost_mouse_pattern3_top150gene_fea,2,as.character)
write.csv2(gost_mouse_pattern3_top150gene_fea, file =  "/Users/eramsey/Desktop/R21_210302/PreliminaryR21/gost_mouse_pattern6_top150gene_fea.csv")
```
...
gost_pattern259 <- dplyr::filter(gost_pattern259, source == "KEGG")
```
...
ggalluvial_KEGG_259 <- ggplot(gost_pattern259,
 ...
 # geom_label(stat = "stratum", label.size = 0.15, aes(label = NULL, size = .5), nudge_x = -0.1) +
  ...
  ggtitle("Pre/Post Exercise-Associated Pattern Pathways")
ggalluvial_KEGG_259
...
```{r}
ggsave("210629_human_patterns_259_kegg_alluvialplot.pdf", ggalluvial_KEGG_259)
```
