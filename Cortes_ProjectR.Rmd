...
params <- setParam(params, "nPatterns", 15)
...
AP_mouse <- CoGAPS(conv_mouse, params, nIterations = 15000)
...
pattern6_human <- dplyr::select(human_feature, Pattern_6) %>% cbind(., StdDev = human_feature_stdev$Pattern_6)
pattern6_human

NMF_projection <- human_NMF$projection
NMF_pval <- human_NMF$pval
NMF_pattern6 <- cbind(NMF_projection)

#look into exclude_iea
gost_res <- gost(query = row.names(human_feature), organism = "hsapiens", ordered_query = FALSE, significant = TRUE, exclude_iea = FALSE, measure_underrepresentation = FALSE, evcodes = FALSE, 
