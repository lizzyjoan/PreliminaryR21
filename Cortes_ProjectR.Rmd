...
human_featloadings_long$log.expression <- log(human_featloadings_long$feat_weight)
```

human Gene/Feature NMF Matrix Heatmap in ggplot
```{r}
human_featloadings_heatmap <- ggplot(human_featloadings_long, mapping = aes(x = pattern, y = genes, fill = log.expression)) + 
  geom_tile() + scale_colour_viridis_c(option = "plasma") + 
  xlab(label = "Patterns") + ylab(label = "Genes") + 
