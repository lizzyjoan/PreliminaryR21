...
human_NMF_boxplot <- ggplot(human_sampleFactors_long, aes(x = Exercise_Pattern, y = Factors)) +
  ...
  labs(x = "Pre and Post Exercise Patterns", y = "Factors") + 
...
```{r eval=FALSE, include=FALSE}
ggsave("/Users/eramsey/Desktop/R21_210302/PreliminaryR21/210621_human_NMF_prepost_pattern_boxplot.pdf", human_NMF_boxplot)
...
#top 100 most variable genes
top100_var_human_featloadings_names <- names(sort(var_human_featloadings, decreasing = TRUE))[1:100] 
head(top100_var_human_featloadings_names)
```
...
top75_human_featloadings_names <- names(sort(avg_human_featloadings, decreasing = TRUE))[1:75]
head(top75_human_featloadings_names)
```

Human - data wrangling
```{r}
#data wrangling
top75_human_featloadings <- human_featloadings[top75_human_featloadings_names,]
top75_human_featloadings <- as.data.frame(top75_human_featloadings)
top75_human_featloadings <- rownames_to_column(top75_human_featloadings, var = "genes")
dim(top75_human_featloadings)
head(top75_human_featloadings)
```
Human  
https://jcoliver.github.io/learn-r/009-expression-heatmaps.html
```{r}
human_featloadings_long <- pivot_longer(data = top75_human_featloadings, 
                         cols = -genes,
                         names_to = "pattern", 
                         values_to = "feat_weight")
...
#plot pca
projection_plot <- ggplot(projection_NMF, aes(x = Pattern_3, y = Pattern_2, colour = PrePost)) + geom_point() + ggtitle("Mouse Data NMF Patterns Projected onto Human Data")
projection_plot
