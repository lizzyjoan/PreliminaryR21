...
human_NMF_boxplot <- ggplot(human_sampleFactors_long, aes(x = Exercise_Pattern, y = Factors)) +
  ...
  labs(x = "Pre and Post Exercise Patterns", y = "Sample Weights") + 
  theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs(title = "Human NMF Pre/Post Exercise Patterns")
human_NMF_boxplot
```

```{r eval=FALSE, include=FALSE}
ggsave("/Users/eramsey/Desktop/R21_210302/PreliminaryR21/210628_human_NMF_prepost_pattern_boxplot_verticlelabels.pdf", human_NMF_boxplot)
...
human_nmf_heatmap2 <-  heatmap.2(top20_var_human_featloadings_matrix,col=viridis, trace='none', margin=c(3,5),
distfun=function(c) as.dist(1-cor(t(c))) ,
cexCol=1,cexRow=.5,scale = "row",
hclustfun=function(x) hclust(x, method="average"), main = "Top 20 Human NMF Varied Gene Weights"
)
...
#plot pca
projection_plot <- ggplot(projection_NMF, aes(x = Pattern_5, y = Pattern_6, colour = PrePost)) + geom_point() + ggtitle("Mouse Data NMF Patterns Projected onto Human Data")
projection_plot
...
ggalluvial_KEGG_259 <- ggplot(gost_pattern259,
 ...
 # geom_label(stat = "stratum", label.size = 0.15, aes(label = NULL, size = .5), nudge_x = -0.1) +
  ...
  ggtitle("Pathways Associated with Human Patterns")
ggalluvial_KEGG_259
...
```{r}
ggsave("210626_human_patterns_259_kegg_alluvialplot.pdf", ggalluvial_KEGG_259)
```
