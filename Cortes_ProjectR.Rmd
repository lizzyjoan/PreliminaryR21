...
mouse_featloadings_long$log.expression <- log(mouse_featloadings_long$feat_weight)
```

Mouse Gene/Feature NMF Matrix Heatmap in ggplot
```{r}
mouse_featloadings_heatmap <- ggplot(mouse_featloadings_long, mapping = aes(x = pattern, y = genes, fill = log.expression)) + geom_tile() + xlab(label = "Patterns") + ylab(label = "Genes") + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
...
```{r}
mouse_KEGG_3thru6 <- filter(mouse_rkc_3thru6, source == "KEGG")
